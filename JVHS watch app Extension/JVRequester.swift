...
open class JVRequester: NSObject {
    ...
    fileprivate var requestType : JVRequestType!
   // private let url = "http://localhost:8080" //local
    fileprivate let url = "http://www.alpine-furnace-774.appspot.com"//production
    fileprivate let sessionConfig: URLSessionConfiguration = URLSessionConfiguration.default
    fileprivate let session : URLSession
    open var delegate : JVRequestDelegate?
    ...
    fileprivate func makeRequest(_ url: URL)
    {
    ...
    open func logIn(_ username: String, password: String){
    ...
    open func getGrades(_ user: User){
